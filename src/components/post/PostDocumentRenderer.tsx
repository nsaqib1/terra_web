...
function InlineText({ nodes }: { nodes: PostTextNode[] }) {
  return (
    <>
      {nodes.map((node, i) => {
        ...
        for (let m = node.marks.length - 1; m >= 0; m--) {
          content = renderMark(node.marks[m], content, `${i}-${m}`);
        }
        return content;
      })}
    </>
...
function ParagraphNode({ node }: { node: PostParagraphNode }) {
...
function HeadingNode({ node }: { node: PostHeadingNode }) {
  const cls = headingClass[node.level];
  const Tag = (`h${node.level + 1}`) as "h2" | "h3" | "h4";
  return (
    <Tag className={cls}>
      <InlineText nodes={node.content} />
    </Tag>
...
function ListItemNode({ node }: { node: PostListItemNode }) {
  return (
...
function BulletListNode({ node }: { node: PostBulletListNode }) {
  return (
...
function OrderedListNode({ node }: { node: PostOrderedListNode }) {
  return (
...
function BlockquoteNode({ node }: { node: PostBlockquoteNode }) {
  return (
...
function CodeBlockNode({ node }: { node: PostCodeBlockNode }) {
  return (
...
function RenderNode({ node }: { node: PostNode }) {
  switch (node.type) {
    case "paragraph":
